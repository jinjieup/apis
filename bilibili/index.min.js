        ...
        if (e.msg == "视频解析失败") {
          ...
          for (let index = 0; index < e.data.list.length; index++) {
            for (let quality = 0; quality < e.data.list[index].accept.length; quality++) {
              q_code[index] += `<span class="badage bg-success" style="padding: 0.3rem;border-radius: 6px;color: white;margin: 0.5rem">${e.data.list[index].accept[quality]}</span>`
            }
              var tag_item = q_code[index].split("undefined");
              console.log("tag_item:",tag_item);
              code += `<div class="col-md-6"><div class="card card-show" style="margin-bottom:1rem;">
              ...
              <div class="card-body">
                ...
                ${tag_item[1]}
                <p class="card-text" style="margin-top:0.5rem;">尺寸：${e.data.list[index].width}x${e.data.list[index].height}</p>
                <p class="card-text">时长：${e.data.list[index].durationFormat}</p>
                <a class="btn btn-primary" href="${e.data.list[index].url}" download="${e.data.title} - ${e.data.list[index].title}" style="display: block;margin: auto;">下载视频</a>
              </div>
            </div></div>`;
          }
          $("#card_show").html("");
